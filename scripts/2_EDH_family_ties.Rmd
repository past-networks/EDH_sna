...
```
```{r}
#filter only one nomen
ulpius_data <- EDH_people_nomen %>% 
  filter(nomen_clean == "Ulpius")

ulpius_data %>% 
  filter(!is.na(nomen_clean)) %>% 
  select(name, nomen_clean) -> ulpius

ulpius %>% 
  count(name, sort = T)

ulp_edges <- as.matrix(ulpius)
ulp <- graph_from_edgelist(ulp_edges, directed = TRUE)
plot(ulp)
```
