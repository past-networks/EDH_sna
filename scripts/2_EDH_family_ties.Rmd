...
```{r}
# people in the lower offices on inscriptions from Pompeii
...
EDH_nomen_lo_pompeii <- EDH_nomen_lo %>% 
  filter(findspot_ancient_clean == "Pompeii") 

# graph from matrix

edges_nomen_lo <- EDH_nomen_lo_pompeii %>% 
  select(name, nomen_clean) %>% 
  as.matrix()

f_nomen_lo <- graph_from_edgelist(edges_nomen_lo, directed = TRUE)
plot(f_nomen_lo)
...
plot(f_nomen_lo, vertex.color = as.factor(V(f_nomen_lo)$nomen_clean))
```
